...
class SplitScriptAwkSection extends SplitScriptSectionBase_1.SplitScriptSectionBase {
    ...
    get_variables() {
        ...
        */
        ...
        while (match = regex_assigned.exec(this.content)) {
            if (match.length > 0) {
                ...
                if (array_match !== null && array_match.index !== undefined) {
                    ...
                    result.push([res_var, match.index + match[0].indexOf(res_var), Section_1.AwkVariableOccurence.assignment]);
        ...
        while (match = regex_assigned_delete.exec(this.content)) {
            if (match.length > 0) {
                ...
                if (arr_match !== null && arr_match.index !== undefined) {
                    ...
                    result.push([res_var, match.index + match[0].indexOf(res_var), Section_1.AwkVariableOccurence.delete]);
                    this.handle_parameters(parameters, match.index + var_name.indexOf(res_var), match[0], result);
                }
                else {
        ...
        while (match = regex_assigned_incdec_suffix.exec(this.content)) {
            if (match.length > 0) {
                let res_var = this.var_cleanup(match[1]);
                result.push([res_var, match.index + match[0].indexOf(res_var), Section_1.AwkVariableOccurence.incremented_decremented]);
            }
        }
        ...
        for (let variable of result) {
            ...
            while ((index = this.content.indexOf(variable[0], index + 1)) > -1) {
                result2.push([variable[0], index, Section_1.AwkVariableOccurence.embedded]);
            }
        }
        if (result2.length > 0) {
    ...
    handle_var(variable, match_index, full_match, result, occurence = undefined) {
        ...
        if (array_match !== null && array_match.length > 1 && array_match.index !== undefined) {
            result.push([array_match[1], full_match.indexOf(array_match[1]), Section_1.AwkVariableOccurence.assignment]);
            ...
            result.push([variable, match_index + full_match.indexOf(variable), occurence || Section_1.AwkVariableOccurence.assignment]);
        }
    }
    handle_parameters(content, match_index, full_match, result) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            this.handle_var(items[i], match_index, full_match, result, Section_1.AwkVariableOccurence.embedded);
        }
    }
}
exports.SplitScriptAwkSection = SplitScriptAwkSection;
//# sourceMappingURL=SplitScriptAwkSection.js.map