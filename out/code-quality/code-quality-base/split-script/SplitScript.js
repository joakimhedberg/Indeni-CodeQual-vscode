...
class SplitScript {
    ...
    get_ip_address(context) {
        return __awaiter(this, void 0, void 0, function* () {
            let ip_history = vscode.workspace.getConfiguration().get('indeni.commandRunnerIPHistory', []);
            let crunner = new CommandRunnerAsync_1.CommandRunnerAsync();
            let items = [];
            ...
            let ip = yield vscode.window.showQuickPick(items, { canPickMany: false, ignoreFocusOut: true, placeHolder: 'Select ip' });
            if (ip === undefined) {
                return Promise.reject();
            }
            else if (ip.label === '<new>') {
                let input = yield vscode.window.showInputBox({ placeHolder: 'IP Address / Server name', ignoreFocusOut: true, prompt: 'Device IP / Server name', validateInput: crunner.validate_ip_address });
                if (input !== undefined) {
